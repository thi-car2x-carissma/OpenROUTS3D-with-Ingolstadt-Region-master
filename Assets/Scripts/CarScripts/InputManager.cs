    ...
    public class InputManager : MonoBehaviour
    {
        ...
        */
        ...
        void Update()
        {
            ...
            // ---------------- G29 (Wheel + Pedals) -> feed delayed pipeline ----------------
            if (!Settings.paused)
            {
                float steer = Input.GetAxis("G29_Steer");   // -1..+1

                float throttle = g29.Driving.Throttle.ReadValue<float>(); // 0..1
                float brake = g29.Driving.Brake.ReadValue<float>();       // 0..1

                // If pedals are reversed, uncomment these two lines:
                // throttle = 1f - throttle;
                // brake = 1f - brake;

                float vertical = throttle - brake; // -1..+1

                if (Mathf.Abs(steer - lastSteer) > g29ChangeThreshold ||
                    Mathf.Abs(throttle - lastThrottle) > g29ChangeThreshold ||
                    Mathf.Abs(brake - lastBrake) > g29ChangeThreshold)
                {
                    ml_inputdirection_timestamp.Add(
                        new Tuple<Vector2, float>(new Vector2(steer, vertical), Time.fixedTime)
                    );
